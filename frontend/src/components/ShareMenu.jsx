...
const ShareMenu = ({ postId, title }) => {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      color: 'text-[#1DA1F2]',
      action: () => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(postUrl)}&text=${encodeURIComponent(title)}`, '_blank')
    },
    {
      name: 'WhatsApp',
      icon: (
      ...
      color: 'text-[#25D366]',
      action: () => window.open(`https://wa.me/?text=${encodeURIComponent(title + ' ' + postUrl)}`, '_blank')
    },
    {
      name: 'LinkedIn',
      icon: (
    ...
    },
    {
      ...
      action: () => window.location.href = `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(postUrl)}`
    },
    {
      name: 'Copy Link',
      icon: (
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900 dark:bg-gray-700 text-white px-6 py-3 rounded-full shadow-lg z-50 flex items-center gap-2"
          >
